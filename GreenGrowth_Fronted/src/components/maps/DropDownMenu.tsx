...
export default function DropDownMenuOfIndustries({ selected, onSelect }: DropDownProps) {
  const industries = [
  ...
  const toggleSelection = (industry: string) => {
    let updated: string[] = [];

    if (selected.includes(industry)) {
      updated = selected.filter(i => i !== industry);
    } else {
      updated = [...selected, industry];
    }
            ...
            <FormControlLabel
              ...
              control={
                <Checkbox
                  ...
                  onChange={() => toggleSelection(industry)}
                />
              }
              label={
                <Typography
